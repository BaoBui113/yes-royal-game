...
export default function HeaderBottom() {
  ...
  return (
    <>
      <div className="xl:block hidden bg-[#181818]">
        <div className="container h-[120px] relative z-10">
          ...
          <ul className="absolute right-0 top-1/2 -translate-y-1/2 text-white sitemenu">
            {listHref.map((item, inx) => {
              return (
                <li
                  className="bg-[url('/assets/image/header/sitemenu.png')] relative h-[60px] float-left"
                  key={inx}
                >
                  <p
                    onClick={() => handleOpenModalHeader(item)}
                    style={{
                      textIndent: '-200%',
                    }}
                    className="cursor-pointer block opacity-0 overflow-hidden absolute top-0 left-0 right-0 bottom-0 bg-[url('/assets/image/header/sitemenu.png')] transition-all ease-in .2s"
                  >
                    {item.title}
                  </p>
                  {item.subHref && (
                      ...
                      {item.subHref.map((subItem, subIdx) => {
                        return (
                          <li key={subIdx}>
                            <p
                              onClick={() => handleOpenModalHeader(item)}
                              className="text-[#7a5a3d] cursor-pointer block h-6 leading-6 overflow-hidden text-xs hover:bg-[#333] hover:text-[#e2c396] w-full"
                            >
                              {subItem.title}
                            </p>
                          </li>
